...
class ITIS(unittest.TestCase):
    ...
    def test_itis_getcommentdetailfromtsn(self):
        ...
        assert set(comments.keys()) == set(
            ["commentator", "comment", "commtime", "updatedate", "commid"]
        )
    ...
    def test_itis_getcommonnamesfromtsn(self):
        ...
        """
        common_names = pytaxize.itis.getcommonnamesfromtsn(
            tsn=180543, as_dataframe=False
        )
        assert set(common_names[1]) == set(["comname", "lang", "tsn"])

        common_names_df = pytaxize.itis.getcommonnamesfromtsn(
            tsn=180543, as_dataframe=True
        )
